...
import com.loopring.looprwalletnetwork.services.LoopringService
...
class LoopringServiceTest {
    ...
    fun setup() {
        ...
        serviceLive = LoopringService("1.0")
    }
    ...
    fun getBalances_shouldWorkMock() = runBlocking {
        val deferred = serviceLive.getBalances("0x847983c3a34afa192cfee860698584c030f4c9db1")
        val result = deferred.await()
        Assert.assertEquals(result.id,64)
        Assert.assertEquals(result.tokens?.get(0)?.token,"LRC")
    }
}